...
main_joint <- left_join(main, select(recipes_final,
...
main_joint$date <- as.Date(main_joint$date)


ts <- ts(main_joint, 
         start = as.numeric(format(min(main_joint$date), "%Y")), frequency = 365)

plot(ts, main = "Total Lettuce Over Time", ylab = "Total Lettuce", xlab = "Time")
